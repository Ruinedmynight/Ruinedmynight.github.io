...
纯静态个人博客，基于 HTML + CSS + JavaScript 构建，部署在 GitHub Pages。文章以 Markdown 编写，前端通过 [marked.js](https://github.com/markedjs/marked) 渲染，无需任何构建工具或后端服务。
...
- **零配置** — 无需 Node.js、Webpack 或其他工具链，纯静态直接运行
...
├── favicon.svg          # 站点图标
├── css/
│   └── style.css        # 全部样式（含亮色/暗色主题变量）
├── js/
│   └── main.js          # 星空粒子、主题切换、文章渲染、搜索、菜单逻辑
├── posts/               # 文章目录
│   └── *.md             # 你的文章（内嵌 YAML 头）
├── images/              # 图片资源
└── README.md
...
2. 在 `js/main.js` 中找到 `POST_SLUGS` 数组，添加文件名（不含 `.md` 后缀）：
